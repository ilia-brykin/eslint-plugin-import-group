    ...
    return {
      Program(node) {
        ...
        const {
          anotherNodesBetweenImports,
          importNodes
        } = getImportBlockNodes({ node, preserveNonImportCode });
        ...
        const textBetweenImports = getTextBetweenImports({ importNodes, sourceCode });
        let fixedCode = sortedImports.join("\n");

        if (preserveNonImportCode && anotherNodesBetweenImports.length) {
          const anotherTextsBetweenImports = getNodeTexts({ nodes: anotherNodesBetweenImports, sourceCode });
